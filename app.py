"""
Map-to-STL v2.1 - Bambu Lab Style
Created by SebGE - https://makerworld.com/de/@SebGE
Now with 3D Terrain Maps!
"""
...
def geocode(q):
    ...
    try:
        r=requests.get('https://nominatim.openstreetmap.org/search',params={'q':q,'format':'json','limit':1},headers={'User-Agent':'MapToSTL/2.1'},timeout=10)
...
def fetch_elevation(lat, lon, radius, resolution=100):
    """Fetch elevation data from Open-Elevation API or generate terrain."""
    ...
    # Try Open-Elevation API (free, no key needed)
    try:
        locations = []
        # Sample fewer points for API (10x10 grid, then interpolate)
        ...
        if response.status_code == 200:
            ...
            print(f"  ✓ Elevation data fetched (min: {elevation_data.min():.0f}m, max: {elevation_data.max():.0f}m)")
...
def generate_procedural_terrain(lat, lon, resolution):
    """Generate realistic procedural terrain using multiple noise octaves."""
    ...
    # Normalize to reasonable elevation range (200-2000m)
...
def create_terrain_mesh(elevation_data, size, height_scale, base_height=2.0):
    ...
    # Normalize elevation to desired height scale
    ...
    # Bottom surface vertices (flat base)
    for i in range(resolution):
        for j in range(resolution):
            x = -half + j * step
            y = -half + i * step
...
def api_terrain_pv():
    try:
        ...
        # Draw contour-like visualization
        res=elev.shape[0]
...
def health():return jsonify({'status':'ok','version':'2.1','author':'SebGE','features':['streets','terrain']})

if __name__=='__main__':
    port=int(os.environ.get('PORT',8080))
    print(f"\n  🗺️  Map → STL v2.1 by SebGE\n  → http://localhost:{port}\n  Features: Street Maps + 3D Terrain\n")
