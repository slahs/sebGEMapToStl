"""
Map-to-STL v2.0 - Bambu Lab Style
...
def geocode(q):
    ...
    try:
        r=requests.get('https://nominatim.openstreetmap.org/search',params={'q':q,'format':'json','limit':1},headers={'User-Agent':'MapToSTL/2.0'},timeout=10)
...
def gen_svg(gdf,lat,lon,radius,lw,size,style='lines',bg='#1a1a1a',fg='#00AE42'):
    ...
    for g in cl.geometry:
        ...
        for ln in(g.geoms if g.geom_type=='MultiLineString' else[g]):
            ...
            c=list(ln.coords)
            if len(c)<2:continue
            d=f"M{tx(*c[0])[0]:.1f},{tx(*c[0])[1]:.1f}"
            for p in c[1:]:d+=f" L{tx(*p)[0]:.1f},{tx(*p)[1]:.1f}"
            paths.append(d)
    if style=='negative':
        ...
        svg+=f'</g></mask></defs><rect width="100%" height="100%" fill="{fg}" mask="url(#m)"/></svg>'
    elif style=='filled':
        ...
        for p in paths:svg+=f'<path d="{p}"/>'
        svg+=f'</g></g><circle cx="{size/2}" cy="{size/2}" r="{size/2-1}" fill="none" stroke="{fg}" stroke-width="1.5"/></svg>'
...
def gen_stl(gdf,lat,lon,radius,lw,size,height,neg=False):
    gp=gdf.to_crs(gdf.estimate_utm_crs())
    ctr=gpd.GeoDataFrame(geometry=gpd.points_from_xy([lon],[lat]),crs='EPSG:4326').to_crs(gp.crs).geometry.iloc[0]
    bb=box(ctr.x-radius,ctr.y-radius,ctr.x+radius,ctr.y+radius);cl=gp.clip(bb)
    ...
    result=box(-size/2,-size/2,size/2,size/2).difference(streets) if neg else streets
    meshes=[]
...
HTML=open(os.path.join(os.path.dirname(__file__),'template.html')).read() if os.path.exists(os.path.join(os.path.dirname(__file__),'template.html')) else '''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Map to STL | by SebGE</title><link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/><style>:root{--b:#00AE42;--bd:#009639;--bg:#0d0d0d;--c:#1a1a1a;--i:#252525;--br:#333;--t:#fafafa;--d:#888}*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:var(--bg);color:var(--t);min-height:100vh}.app{display:grid;grid-template-columns:420px 1fr 320px;min-height:100vh}.sb{background:var(--c);border-right:1px solid var(--br);padding:16px;display:flex;flex-direction:column;gap:12px;overflow-y:auto}.hd{display:flex;align-items:center;gap:12px;padding-bottom:12px;border-bottom:1px solid var(--br)}.logo{width:44px;height:44px;background:linear-gradient(135deg,var(--b),var(--bd));border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:22px}.hd h1{font-size:18px;font-weight:700}.hd .sub{font-size:10px;color:var(--b);font-weight:500}.cr{display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(0,174,66,.1),rgba(0,174,66,.05));border:1px solid rgba(0,174,66,.3);border-radius:8px;padding:10px 12px;text-decoration:none}.cr:hover{background:rgba(0,174,66,.15);border-color:var(--b)}.cr-av{width:32px;height:32px;background:var(--b);border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;color:#fff}.cr-i{flex:1}.cr-n{font-size:12px;font-weight:600}.cr-l{font-size:10px;color:var(--b)}.cr-bd{font-size:8px;background:var(--b);color:#fff;padding:2px 6px;border-radius:4px;font-weight:600}.sw{position:relative}.si{width:100%;padding:12px 14px 12px 40px;background:var(--i);border:1px solid var(--br);border-radius:8px;color:var(--t);font-size:13px}.si:focus{outline:none;border-color:var(--b)}.sic{position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--d)}.sbt{position:absolute;right:6px;top:50%;transform:translateY(-50%);padding:6px 14px;background:var(--b);border:none;border-radius:6px;color:#fff;font-size:11px;font-weight:600;cursor:pointer}.sbt:hover{background:var(--bd)}.cds{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px}.cd{background:var(--i);border:1px solid var(--br);border-radius:6px;padding:8px 10px;font-family:monospace;font-size:11px}.cd label{display:block;font-size:8px;color:var(--d);text-transform:uppercase;margin-bottom:2px;font-family:sans-serif}.cd span{color:var(--b);font-weight:500}.sec{background:var(--i);border:1px solid var(--br);border-radius:10px;padding:14px}.st{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--b);margin-bottom:12px;display:flex;align-items:center;gap:8px}.st::before{content:"";width:4px;height:14px;background:var(--b);border-radius:2px}.prs{display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap}.pr{padding:6px 10px;background:var(--c);border:1px solid var(--br);border-radius:6px;color:var(--t);font-size:10px;cursor:pointer;font-weight:500}.pr:hover{border-color:var(--b);color:var(--b)}.pr.a{background:var(--b);border-color:var(--b);color:#fff}.tps{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.tp{display:flex;align-items:center;gap:5px;padding:6px 8px;background:var(--c);border:1px solid var(--br);border-radius:6px;cursor:pointer;font-size:10px}.tp:hover{border-color:var(--b)}.tp.a{border-color:var(--b);background:rgba(0,174,66,.1)}.tp .ck{width:12px;height:12px;border:2px solid var(--br);border-radius:3px;font-size:8px;display:flex;align-items:center;justify-content:center}.tp.a .ck{background:var(--b);border-color:var(--b);color:#fff}.stl{display:grid;grid-template-columns:repeat(5,1fr);gap:5px}.sty{padding:10px 6px;background:var(--c);border:2px solid var(--br);border-radius:8px;cursor:pointer;text-align:center}.sty:hover{border-color:var(--b)}.sty.a{border-color:var(--b);background:rgba(0,174,66,.1)}.sty .ic{font-size:20px}.sty .nm{font-size:9px;margin-top:4px;font-weight:500}.cls{display:flex;gap:10px;margin-top:10px}.clp{display:flex;align-items:center;gap:6px;flex:1}.clp label{font-size:10px;color:var(--d);font-weight:500}.clp input[type="color"]{width:32px;height:24px;border:none;border-radius:4px;cursor:pointer;background:none}.cld{width:18px;height:18px;border-radius:4px;cursor:pointer;border:2px solid transparent}.cld:hover{transform:scale(1.15)}.pm{margin-bottom:10px}.pmh{display:flex;justify-content:space-between;align-items:center;margin-bottom:4px}.pml{font-size:11px;font-weight:500}.pmv{font-family:monospace;font-size:10px;color:var(--b);background:var(--c);padding:3px 8px;border-radius:4px;font-weight:600}input[type="range"]{width:100%;height:6px;background:var(--c);border-radius:3px;-webkit-appearance:none;margin-top:2px}input[type="range"]::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;background:var(--b);border-radius:50%;cursor:pointer}.ex{margin-top:auto;padding-top:14px;border-top:1px solid var(--br)}.bts{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:8px}.btn{padding:12px;border:none;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px}.bp{background:linear-gradient(135deg,var(--b),var(--bd));color:#fff;box-shadow:0 4px 12px rgba(0,174,66,.3)}.bp:hover{transform:translateY(-2px)}.bs{background:var(--i);color:var(--t);border:1px solid var(--br)}.bs:hover{border-color:var(--b);color:var(--b)}.btn:disabled{opacity:.5;cursor:not-allowed}.sp{width:14px;height:14px;border:2px solid transparent;border-top-color:currentColor;border-radius:50%;animation:spin .7s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.sts{padding:10px;border-radius:6px;font-size:11px;margin-top:8px;display:none;font-weight:500}.sts.er{display:block;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);color:#ef4444}.sts.ok{display:block;background:rgba(0,174,66,.1);border:1px solid rgba(0,174,66,.3);color:var(--b)}.mw{position:relative}#map{width:100%;height:100%;background:var(--i)}.mi{position:absolute;top:12px;right:12px;background:rgba(26,26,26,.95);border:1px solid var(--br);border-radius:8px;padding:10px 14px;font-size:10px;color:var(--d);z-index:1000}.pv{background:var(--c);border-left:1px solid var(--br);padding:16px;display:flex;flex-direction:column;gap:14px;overflow-y:auto}.pv h2{font-size:13px;font-weight:700}.bdg{font-size:9px;padding:3px 8px;background:var(--b);color:#fff;border-radius:6px;margin-left:auto;font-weight:600}.pvb{background:var(--i);border:1px solid var(--br);border-radius:12px;aspect-ratio:1;display:flex;align-items:center;justify-content:center;overflow:hidden}#pvs{width:100%;height:100%}.ph{color:var(--d);font-size:11px;text-align:center}.sts2{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.sta{background:var(--i);border:1px solid var(--br);border-radius:8px;padding:8px;text-align:center}.sta label{font-size:8px;color:var(--d);text-transform:uppercase}.sta .vl{font-family:monospace;font-size:12px;color:var(--b);margin-top:3px;font-weight:600}.inf{font-size:10px;color:var(--d);line-height:1.5;background:var(--i);border-radius:8px;padding:12px}.inf strong{color:var(--b)}.ft{margin-top:auto;padding-top:12px;border-top:1px solid var(--br);text-align:center;font-size:9px;color:var(--d)}.ft a{color:var(--b);text-decoration:none}.ft a:hover{text-decoration:underline}@media(max-width:1300px){.app{grid-template-columns:400px 1fr}.pv{display:none}}@media(max-width:700px){.app{grid-template-columns:1fr;grid-template-rows:35vh auto}.sb{order:2}.mw{order:1}}</style></head><body><div class="app"><aside class="sb"><div class="hd"><div class="logo">🗺️</div><div><h1>Map → STL</h1><div class="sub">3D PRINT YOUR CITY</div></div></div><a href="https://makerworld.com/de/@SebGE" target="_blank" class="cr"><div class="cr-av">S</div><div class="cr-i"><div class="cr-n">Created by SebGE</div><div class="cr-l">makerworld.com/@SebGE</div></div><div class="cr-bd">MAKER</div></a><div><div class="sw"><svg class="sic" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg><input type="text" id="loc" class="si" placeholder="Search city..." value="Düsseldorf"><button class="sbt" onclick="search()">Search</button></div><div class="cds"><div class="cd"><label>Lat</label><span id="lv">51.2277</span></div><div class="cd"><label>Lon</label><span id="lov">6.7735</span></div></div></div><div class="sec"><div class="st">Street Types</div><div class="prs"><button class="pr a" onclick="preset('city')">🏙️ City</button><button class="pr" onclick="preset('rural')">🌾 Rural</button><button class="pr" onclick="preset('all')">📍 All</button><button class="pr" onclick="preset('main')">🛣️ Main</button><button class="pr" onclick="preset('paths')">🚶 Paths</button><button class="pr" onclick="preset('none')">❌ Clear</button></div><div class="tps" id="tps"><label class="tp a" data-t="motorway"><span class="ck">✓</span>🚀 Highway</label><label class="tp a" data-t="trunk"><span class="ck">✓</span>🛣️ Trunk</label><label class="tp a" data-t="primary"><span class="ck">✓</span>🔴 Primary</label><label class="tp a" data-t="secondary"><span class="ck">✓</span>🟠 Secondary</label><label class="tp a" data-t="tertiary"><span class="ck">✓</span>🟡 Tertiary</label><label class="tp a" data-t="residential"><span class="ck">✓</span>🏠 Residential</label><label class="tp" data-t="unclassified"><span class="ck">✓</span>📍 Unclass.</label><label class="tp" data-t="service"><span class="ck">✓</span>🅿️ Service</label><label class="tp" data-t="track"><span class="ck">✓</span>🌾 Field path</label><label class="tp" data-t="footway"><span class="ck">✓</span>🚶 Footway</label><label class="tp" data-t="cycleway"><span class="ck">✓</span>🚴 Cycleway</label><label class="tp" data-t="path"><span class="ck">✓</span>🥾 Path</label><label class="tp" data-t="pedestrian"><span class="ck">✓</span>🚶‍♂️ Pedestrian</label><label class="tp" data-t="bridleway"><span class="ck">✓</span>🐴 Bridleway</label></div></div><div class="sec"><div class="st">Output Style</div><div class="stl"><div class="sty a" data-s="lines" onclick="setStyle('lines')"><div class="ic">〰️</div><div class="nm">Lines</div></div><div class="sty" data-s="filled" onclick="setStyle('filled')"><div class="ic">⬛</div><div class="nm">Filled</div></div><div class="sty" data-s="negative" onclick="setStyle('negative')"><div class="ic">🔲</div><div class="nm">Negative</div></div><div class="sty" data-s="outline" onclick="setStyle('outline')"><div class="ic">⭕</div><div class="nm">Circle</div></div><div class="sty" data-s="transparent" onclick="setStyle('transparent')"><div class="ic">🔳</div><div class="nm">Trans</div></div></div><div class="cls"><div class="clp"><label>Lines:</label><input type="color" id="fg" value="#00AE42"><div class="cld" style="background:#00AE42" onclick="$('fg').value='#00AE42'"></div><div class="cld" style="background:#fff" onclick="$('fg').value='#ffffff'"></div><div class="cld" style="background:#3b82f6" onclick="$('fg').value='#3b82f6'"></div></div><div class="clp"><label>BG:</label><input type="color" id="bg" value="#1a1a1a"><div class="cld" style="background:#1a1a1a" onclick="$('bg').value='#1a1a1a'"></div><div class="cld" style="background:#fff" onclick="$('bg').value='#ffffff'"></div><div class="cld" style="background:#000" onclick="$('bg').value='#000000'"></div></div></div></div><div class="sec"><div class="st">Parameters</div><div class="pm"><div class="pmh"><span class="pml">Radius</span><span class="pmv" id="rv">500m</span></div><input type="range" id="rad" min="100" max="5000" value="500" step="50" oninput="$('rv').textContent=this.value+'m';uc()"></div><div class="pm"><div class="pmh"><span class="pml">Line Width</span><span class="pmv" id="lwv">1.2mm</span></div><input type="range" id="lw" min="0.3" max="4" value="1.2" step="0.1" oninput="$('lwv').textContent=parseFloat(this.value).toFixed(1)+'mm'"></div><div class="pm"><div class="pmh"><span class="pml">Height</span><span class="pmv" id="hv">2.0mm</span></div><input type="range" id="ht" min="0.4" max="8" value="2.0" step="0.1" oninput="$('hv').textContent=parseFloat(this.value).toFixed(1)+'mm'"></div><div class="pm"><div class="pmh"><span class="pml">Size</span><span class="pmv" id="sv">80mm</span></div><input type="range" id="sz" min="20" max="300" value="80" step="5" oninput="$('sv').textContent=this.value+'mm';$('ssz').textContent=this.value+'mm'"></div></div><div class="ex"><div class="bts"><button class="btn bs" id="bp" onclick="preview()">👁 Preview</button><button class="btn bs" id="bsv" onclick="expSVG()">📄 SVG</button><button class="btn bp" id="bst" onclick="expSTL()">⬇ STL</button></div><div class="bts"><button class="btn bs" style="font-size:10px" onclick="expSTL(false)">STL Normal</button><button class="btn bs" style="font-size:10px" onclick="expSTL(true)">STL Negative</button></div><div class="sts" id="sts"></div></div><div class="ft">Made with ❤️ for 3D printing<br><a href="https://makerworld.com/de/@SebGE" target="_blank">@SebGE on MakerWorld</a></div></aside><main class="mw"><div id="map"></div><div class="mi">🖱️ Click to set center</div></main><aside class="pv"><h2>Preview <span class="bdg" id="bdg" style="display:none">Ready</span></h2><div class="pvb"><div id="pvs"></div><div class="ph" id="ph">👁 Click Preview</div></div><div class="sts2"><div class="sta"><label>Segments</label><div class="vl" id="sseg">—</div></div><div class="sta"><label>Size</label><div class="vl" id="ssz">80mm</div></div><div class="sta"><label>Style</label><div class="vl" id="sst">Lines</div></div></div><div class="inf"><strong>Export:</strong><br>• SVG → Laser/Print<br>• STL Normal → Raised<br>• STL Negative → Cut-out</div></aside></div><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><script>const $=id=>document.getElementById(id);let map,mk,cc,lat=51.2277,lon=6.7735,sty='lines';const prs={city:['motorway','trunk','primary','secondary','tertiary','residential'],rural:['primary','secondary','tertiary','residential','track','unclassified','path','service'],all:['motorway','trunk','primary','secondary','tertiary','residential','unclassified','service','track','footway','cycleway','path','pedestrian','bridleway'],main:['motorway','trunk','primary','secondary'],paths:['footway','cycleway','path','pedestrian','bridleway','track'],none:[]};function init(){map=L.map('map').setView([lat,lon],14);L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',{maxZoom:19}).addTo(map);um();map.on('click',e=>{lat=e.latlng.lat;lon=e.latlng.lng;um();uco()});document.querySelectorAll('.tp').forEach(el=>el.onclick=()=>{el.classList.toggle('a');document.querySelectorAll('.pr').forEach(p=>p.classList.remove('a'))});$('loc').onkeypress=e=>{if(e.key==='Enter')search()}}function preset(n){const t=prs[n]||[];document.querySelectorAll('.tp').forEach(el=>el.classList.toggle('a',t.includes(el.dataset.t)));document.querySelectorAll('.pr').forEach(p=>p.classList.toggle('a',p.textContent.toLowerCase().includes(n)))}function setStyle(s){sty=s;document.querySelectorAll('.sty').forEach(el=>el.classList.toggle('a',el.dataset.s===s));$('sst').textContent=s.charAt(0).toUpperCase()+s.slice(1)}function gt(){return Array.from(document.querySelectorAll('.tp.a')).map(e=>e.dataset.t)}function um(){const r=+$('rad').value;if(mk)map.removeLayer(mk);if(cc)map.removeLayer(cc);mk=L.marker([lat,lon],{icon:L.divIcon({html:'<div style="width:16px;height:16px;background:#00AE42;border:2px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(0,174,66,.5)"></div>',iconSize:[16,16],iconAnchor:[8,8]})}).addTo(map);cc=L.circle([lat,lon],{radius:r,color:'#00AE42',fillOpacity:.08,weight:2,dashArray:'8,8'}).addTo(map)}function uc(){const r=+$('rad').value;if(cc)cc.setRadius(r)}function uco(){$('lv').textContent=lat.toFixed(5);$('lov').textContent=lon.toFixed(5)}function pm(){return{lat,lon,radius:+$('rad').value,line_width:+$('lw').value,extrusion_height:+$('ht').value,target_size:+$('sz').value,street_types:gt(),style:sty,bg_color:$('bg').value,line_color:$('fg').value}}async function search(){const q=$('loc').value.trim();if(!q)return;try{const r=await fetch('/api/geocode?q='+encodeURIComponent(q));const d=await r.json();if(d.error)throw new Error(d.error);lat=d.lat;lon=d.lon;map.setView([lat,lon],14);um();uco();msg('ok','✓ '+(d.name||q).substring(0,40))}catch(e){msg('er',e.message)}}async function preview(){const btn=$('bp');btn.disabled=true;btn.innerHTML='<div class="sp"></div>';$('bdg').textContent='...';$('bdg').style.display='inline';try{const r=await fetch('/api/preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(pm())});const d=await r.json();if(d.error)throw new Error(d.error);$('ph').style.display='none';$('pvs').innerHTML=d.svg;$('sseg').textContent=d.stats.segments;$('bdg').textContent='Ready';msg('ok','✓ '+d.stats.segments+' segments')}catch(e){msg('er',e.message);$('bdg').style.display='none'}finally{btn.disabled=false;btn.innerHTML='👁 Preview'}}async function expSVG(){const btn=$('bsv');btn.disabled=true;btn.innerHTML='<div class="sp"></div>';try{const r=await fetch('/api/svg',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(pm())});if(!r.ok)throw new Error((await r.json()).error);dl(await r.blob(),`map_${lat.toFixed(4)}_${lon.toFixed(4)}_${sty}.svg`);msg('ok','✓ SVG')}catch(e){msg('er',e.message)}finally{btn.disabled=false;btn.innerHTML='📄 SVG'}}async function expSTL(neg=false){const btn=$('bst');btn.disabled=true;btn.innerHTML='<div class="sp"></div>';try{const p={...pm(),negative:neg};const r=await fetch('/api/stl',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)});if(!r.ok)throw new Error((await r.json()).error);dl(await r.blob(),`map_${lat.toFixed(4)}_${lon.toFixed(4)}_${neg?'negative':'normal'}.stl`);msg('ok','✓ STL')}catch(e){msg('er',e.message)}finally{btn.disabled=false;btn.innerHTML='⬇ STL'}}function dl(b,n){const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=n;a.click();URL.revokeObjectURL(a.href)}function msg(t,m){const el=$('sts');el.className='sts '+t;el.textContent=m;setTimeout(()=>el.className='sts',5000)}document.addEventListener('DOMContentLoaded',init)</script></body></html>'''
...
def api_pv():
    try:
        ...
        svg=gen_svg(gdf,d['lat'],d['lon'],d['radius'],d['line_width'],d['target_size'],d.get('style','lines'),d.get('bg_color','#1a1a1a'),d.get('line_color','#00AE42'))
...
def api_stl():
    try:
        d=request.json;gdf=fetch_streets(d['lat'],d['lon'],d['radius'],d.get('street_types'))
        mesh=gen_stl(gdf,d['lat'],d['lon'],d['radius'],d['line_width'],d['target_size'],d['extrusion_height'],d.get('negative',False))
...
def health():return jsonify({'status':'ok','version':'2.0','author':'SebGE'})

if __name__=='__main__':
    port=int(os.environ.get('PORT',8080))
    print(f"\n  🗺️  Map → STL v2.0 by SebGE\n  → http://localhost:{port}\n")
